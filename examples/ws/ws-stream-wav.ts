...
async function main() {
  ...
  const FILE = env('FILE') ?? join(process.cwd(), 'examples', 'assets', 'sample-speech.wav');
  ...
  for await (const frame of reader.read({ frameDurationMs: 200 })) {
    ...
    if (sent % 25 === 0) {
      console.debug('L16 egress frame size', { sampleRate: frame.sampleRate, bytes, samples, frameMs: Math.round(frameMs) });
    }
